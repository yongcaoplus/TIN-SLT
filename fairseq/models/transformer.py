...
class TransformerS2Encoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        bert_gates = getattr(args, 'en_bert_gates', [1, 1, 1, 1, 1, 1])
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        bert_gates = getattr(args, 'de_bert_gates', [1, 1, 1, 1, 1, 1])
        bert_gates = [x == 1 for x in bert_gates]
...
class TransformerS2EncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, bert_gate=True):
        ...
        # instruction module
        if self.args.instruction_layer_num > 0:
            self.instruction_encoder = Linear(self.embed_dim, self.embed_dim)
        ...
        # ######## 更改bert ratio 添加方式  ######
        if self.args.alpha_mode != 'constant':
            self.bert_weight = torch.nn.Parameter(torch.FloatTensor([self.bert_ratio]), requires_grad=True)
        # ######## END 更改bert ratio 添加方式  ######
        ...
        self.ablation_experiment = getattr(args, 'ablation_experiment', "all")
    ...
    def forward(self, x, encoder_padding_mask, bert_encoder_out, bert_encoder_padding_mask):
        ...
        # instruction module
        if self.args.instruction_layer_num > 0:
            x2 = self.instruction_encoder(x2)
        x2 = F.dropout(x2, p=self.dropout, training=self.training)
        ratios = self.get_ratio()
        # import pdb;pdb.set_trace()
        # print("----------------------------- set constnt")
        if self.ablation_experiment == "none" or self.ablation_experiment == "only_decoder":
            x = residual + x1
        elif "alpha_mode" in self.args and self.args.alpha_mode:
            # print("Encoder bert weight . ", self.bert_weight.data)
            if self.args.alpha_mode == "strategy":
                x = residual + (1 - self.args.alpha_bert) * x1 + self.args.alpha_bert * x2
            elif self.args.alpha_mode == "constant":
                # print("ratios are: ", ratios)
                x = residual + ratios[0] * x1 + ratios[1] * x2
            elif self.args.alpha_mode == "learn":
                x = residual + (1 - self.bert_weight) * x1 + self.bert_weight * x2
        else:
            x = residual + ratios[0] * x1 + ratios[1] * x2
        x = self.maybe_layer_norm(self.self_attn_layer_norm, x, after=True)
    ...
    def get_ratio(self):
        if self.encoder_bert_dropout:
            ...
            return [1 - self.bert_ratio, self.bert_ratio]
...
class TransformerDecoderLayer(nn.Module):
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, no_encoder_attn=False, add_bias_kv=False, add_zero_attn=False, bert_gate=True):
        ...
        ## instruction module
        if self.args.instruction_layer_num > 0:
            self.instruction_decoder = Linear(self.embed_dim, self.embed_dim)
            # end instruction module
    ...
    def forward(
        ...
        if self.encoder_attn is not None:
            ...
            ## add instruction
            if self.args.instruction_layer_num > 0:
                x2 = self.instruction_decoder(x2)
            x2 = F.dropout(x2, p=self.dropout, training=self.training)
            ratios = self.get_ratio()

            # print("Decoder bert weight . ", self.bert_weight.data)
            if self.ablation_experiment == "none" or self.ablation_experiment == "only_encoder":
                x = residual + x1
            elif "alpha_mode" in self.args and self.args.alpha_mode:
                if self.args.alpha_mode == "strategy":
                    x = residual + (1 - self.args.alpha_bert) * x1 + self.args.alpha_bert * x2
